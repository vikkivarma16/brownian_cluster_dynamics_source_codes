...
long int tsim, ci, dpuc, dpuc1, ve, vel, sjet, tsjet;
...
int main(void)
{
	...
	printf("Type of patches defined is given as (not necessarily equipped on the particles): %d \n\n\n\n", tpatch);
	...
	for (i=0; i<tpatch; i++)
	{
		for (j=0; j<tpatch; j++)
		{
			 ...
			 else
			 {
			 	printf("Epsi-   %lf\n", epsi2[i][j]);
			 	printf("Beta-   %lf\n", beta2[i][j]);
			 	printf("Temperature K_bT/u_0 =   %lf \n",-1.0/ur[i][j]);
	...
	*/
	...
	//Preparing for simulation!
     	...
     	FILE* lwu;
     	lwu=fopen("Movement_per_10_thousand_unit_simulation_time.txt","w");
     	fclose (lwu);
     	//FILE* dwu;
	...
	printf("Data will be printed after each %d simulation time..    \n", lsd);
	printf("\n");
	printf("\n");

	printf("Maximum possible data set with given similation time is given by-   ");
	printf("%d\n",iltt);
	printf("\n");
	printf("\n");
	...
	{	
		...
		{	
			...
			for(mo=0; mo<nlies; mo++)
			{
				...
				{
					if (sitt>=probation) tsjet=tsjet+1;
					//printf("Flag_trans\n");
					trim=2;
					...
					istep=vstep*(JKISS() / 4294967296.0);
					
					dx = istep*sin(theta)*cos(phi);
					dy = istep*sin(theta)*sin(phi); 
					dz = istep*cos(theta);  
					...
					if (pop[it][jt]==1)
					{
						...
						for (i=0; i<N; i++)
						{
							...
							for (j=0; j<wcbobb[i]; j++)
							{
								...
								dxi=ami[cbobb[i][j][0]][0]+v1*ec*0.5;
								dyi=ami[cbobb[i][j][0]][1]+v2*ec*0.5;
								dzi=ami[cbobb[i][j][0]][2]+v3*ec*0.5;
